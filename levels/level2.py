...
class Level2(BaseScreen):
    ...
    def replicate_pathogens(self):
        ...
        # Check if 10 seconds have passed since the last replication
        if not hasattr(self, "last_replicate_time"):
            self.last_replicate_time = current_time  # Initialize if not already set

        if current_time - self.last_replicate_time < 10000:  # 10 seconds = 10000 ms
            return  # Skip replication if 10 seconds haven't passed

        # Filter eligible pathogens (exclude replicas and ensure they can replicate)
        eligible_pathogens = [
            enemy for enemy in self.enemies
            if enemy.can_replicate and enemy.replicas < 2
        ]

        if eligible_pathogens:  # Ensure there are eligible pathogens
            # Randomly pick one pathogen to replicate
            enemy = random.choice(eligible_pathogens)
            ...
            if target_cell:
                ...
                for i in range(2):  # Create up to 2 replicas
        ...
        # Update the last replicate time
        self.last_replicate_time = current_time
